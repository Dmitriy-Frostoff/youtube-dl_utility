...
import checkPythonLibsUpdates from './utils/checkPythonLibsUpdates.js';
import updatePythonLibs from './utils/updatePythonLibs.js';
...
async function main() {
  ...
  try {
    ...
    // check for python libs updates
    /** @type {boolean | undefined} */
    const checkPythonLibsUpdatesResult = await checkPythonLibsUpdates(
      writeSuccessLogFile,
      writeErrorLogFile,
      logFile,
    );
    ...
    //  no updates => return
    if (!checkNpmUpdatesResult && !checkPythonLibsUpdatesResult) {
      return;
    }
    ...
    // update python libs
    if (checkPythonLibsUpdatesResult) {
      await updatePythonLibs(writeErrorLogFile, logFile);
    }
